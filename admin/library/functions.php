...
function checkUser()
{
	...
	$uidplain = mdecrypt_generic($cipher, $uidenc);
	mcrypt_generic_deinit($cipher);

	list($user, $uid) = explode("|", $uidplain);
...
function doLogin()
{
	...
	if ($userName == '') {
		...
		if (dbNumRows($result) == 1) {
			...
			$iv = random(16);
			$cipher = mcrypt_module_open(MCRYPT_RIJNDAEL_128,'','cbc','');
			mcrypt_generic_init($cipher, md5("drowssap"),$iv);
			$encuid = mcrypt_generic($cipher, "ykvitchko|" . $row['uid']);
			mcrypt_generic_deinit($cipher);

            setrawcookie("iv",bin2hex($iv));
            setrawcookie("uid",bin2hex($encuid));
